...
NPC::NPC(OpenGLContext* context, glm::vec3 pos, Chunk* chunk, Model* model, int modelIndex) :
...
//            } while (glm::length(normal) < FLT_EPSILON && edge != curr);
           glm::vec4 position = curr->vert->pos;
           float scale = modelIndex == 0 ? 0.015f : 0.075f;
           int rotOffset = modelIndex == 0 ? 90 : 180;
           position[0] *= scale;
           position[1] *= scale;
           position[2] *= scale;
           position = glm::rotate(glm::mat4(1.0f), glm::radians((float)rotDegrees + rotOffset), glm::vec3(0.0f, 1.0f, 0.0f)) * position;
           position += glm::vec4(m_position, 0);
...
void NPC::tick(float dT, const Terrain &terrain) {
    if (ticksSinceLastDecision == ticksUntilNextDecision) {
        ...
        ticksUntilNextDecision = rand() % 300;
    }
...
//        }
        m_velocity *= modelIndex != 5 ? 0.66 : 1.33;
        //constant vertical swimming speed
        m_velocity.y = modelIndex != 5 ? 5.f : 0.f;
...
//    return false;
    ...
    try {
        ...
        return currBlock != EMPTY && currBlock != WATER && currBlock != LAVA && currBlock != GRASSDEC && currBlock != SHRUB && currBlock != DAISY && currBlock != ROSE && currBlock != REDMUSH && currBlock != BROWNMUSH;
...
bool NPC::gridMarch(glm::vec3 rayOrigin, glm::vec3 rayDirection, const Terrain &terrain, float *out_dist, glm::ivec3 *out_blockHit)
{
    ...
    while (curr_t < maxLen)
    {
        ...
        if (cellType != EMPTY && cellType != WATER && cellType != LAVA && cellType != GRASSDEC && cellType != SHRUB && cellType != REDMUSH && cellType != BROWNMUSH)
        {
            *out_blockHit = currCell;
            *out_dist = glm::min(maxLen, curr_t);
            return true;
