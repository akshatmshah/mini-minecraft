...
private:
    ...
    //blocktype worker
    std::vector<Chunk*> chunkBlockType;
    QMutex mutexBlock;
    //VBO worker
    std::vector<ChunkData> chunkVBOs;
    QMutex mutexChunk;
    const std::vector<uPtr<Model>> &models;

public:
    Terrain(OpenGLContext *context, const std::vector<uPtr<Model>> &models);
    ~Terrain();
    ...
    void generateTerrain(glm::vec3 currentPos, glm::vec3 prevPos);

    void CreateRiver();

    QSet<int64_t> getNeighborZones(glm::vec2 currZone);

    void createBlockTypeWorker(int64_t val);
    void createVBOWorker(Chunk* currChunk);

    void checkDataStates();
};
